...
export const saveTypingTest = async (req: Request, res: Response) => {
  try {
    ...
    db.prepare("INSERT INTO TypingTest (user_id, typingMode, typingEndMode, errorCorrectionMode, timeStarted, timeEnded, timeTaken, wpm, accuracy, language, targetText) VALUES(?,?,?,?,?,?,?,?,?,?,?)").run(res.locals.user?.id, req.body.typingMode, req.body.typingEndMode, req.body.errorCorrectionMode, req.body.timeStarted, req.body.timeEnded, req.body.timeTaken, req.body.wpm, req.body.accuracy, req.body.language, req.body.targetText);
  }
  catch (e) {
    console.error(e)
