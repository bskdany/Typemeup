...
// import { generateEmptyFingersStatistics } from "./utils";

export function analyse(targetTextByWord: string[], userTypedText: string[], fingerMap: string[][], defaultFingerPositions: string[]) {
  const reversedFingerMap = reverseFingerMap(fingerMap);

  // const fingersStatistics: FingerStatistics[] = generateEmptyFingersStatistics(fingerMap);
  ...
  const fingerKeypressData: FingerKeypressData[] = constructFingerKeypressData([...alignedTargetText], [...alignedUserTypedText], fingerMap, reversedFingerMap, defaultFingerPositions);

  // step 3, doing additional processing to fingerKeypressData to get out more information
  calculateWhenWasLastKeyPressedSameFinger(fingerKeypressData, fingerMap.length);

  return fingerKeypressData;
}
...
function constructFingerKeypressData(alignedTargetText: string[], alignedUserTypedText: string[], fingerMap: string[][], reversedFingerMap: Map<string, number>, defaultFingerPositions: string[]) {
...
function calculateWhenWasLastKeyPressedSameFinger(fingerKeypressData: FingerKeypressData[], fingerMapLength: number) {
  const currentFingerPositionLastKeyPressed: number[] = new Array(fingerMapLength).fill(0);

  for (const fingerKeypress of fingerKeypressData) {
    fingerKeypress.whenWasLastKeyPressed = currentFingerPositionLastKeyPressed[fingerKeypress.fingerNumber];

    if (fingerKeypress.isCorrect) {
      currentFingerPositionLastKeyPressed[fingerKeypress.fingerNumber] = -1;
